...
import static com.example.schoolairdroprefactoredition.utils.FileUtil.addFileWithPath;
...
public class SellingAddNewImpl implements ISellingAddNewPresenter {
    ...
    public void submitNewItem(String token, String cover, List<String> picSet,
        ...
        MultipartBody.Part goodsCover;

        try {
            builder = addFileWithPath(context, "goods_img_set", picSet, true);
            goodsCover = createFileWithPath(context, "goods_img_cover", cover, true);
        } catch (IOException e) {
            if (mCallback != null) {
        ...
        if (goodsCover != null) {
            builder.addPart(goodsCover);
        }

        builder.addPart(goodsName)
                .addPart(goodsDes)
                .addPart(goodsBrandNew)
                .addPart(goodsQuotable)
                .addPart(goodsPrice);

        retrofit2.Call<DomainResult> task = api.postNewItem(builder.build());
...
//                        }
    ...
    @Override
    public void updateItemInfo(String token, String cover, List<String> picSet, String title, String description, double longitude, double latitude, boolean isBrandNew, boolean isQuotable, float price, Context context) {
        Retrofit retrofit = RetrofitManager.getInstance().getDemoRetrofit();
        Api api = retrofit.create(Api.class);
